...
| `NEXT_PUBLIC_PRIVACY_CONTACT_EMAIL`    | Optional. Contact address shown on `/privacy` for privacy/data-protection requests. Empty by default — `/privacy` renders an honest "not yet configured" notice instead of a broken/placeholder contact until a real, monitored mailbox is set here. **A required manual action before public launch.**                                                                                                            |
